...
$query="SELECT utente, cognome, nome, mail, data, indirizzo, citta, cap, id, importo, spedizione FROM acquisti WHERE utente = '$id'";
...
while($art[$i])
{
	$querytit="SELECT titolo FROM ecommerce WHERE codice = '$art[$i]'";
	$arttit= mysql_query($querytit);
	$artic = mysql_fetch_row($arttit);
	$articoli = $articoli."<br>".$artic[0];
	$i++;
}*/
...
<div class="float100"> <h5>Spedizione:</h5></div>
<div class="float100">  <?echo "$utente[10]"?> &euro;</div>
<div class="float100"> <h5>Importo Totale:</h5></div>
<div class="float100">  <?echo "$utente[9]"?> &euro;</div>
<div class="azzerafloat"><br /></div>
</div><br>
...
?>
<div class="azzerafloat"></div><br /><br />
<div class="float70"><img src="./img/ind.png" class="ico" /><a title="|Torna Indietro" href="area.php?pag=gestione_acquisti.php">Indietro</a></div>
<br>					
</div>
