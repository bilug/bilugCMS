...
@$locazione = apici( $_POST['locazione'] );
@$posizione = $_POST['posizione'];
@$marker = $_POST['marker'];

$maps = "$locazione||$posizione||$marker";

if ( $titolo=="" OR $corpo=="" OR $locazione=="" )
{
	$tipoerr="ERRORE: NON HAI INSERITO UN CAMPO OBBLIGATORIO";
	//echo "ERRORE: NON HAI INSERITO UN CAMPO OBBLIGATORIO";
	//echo "<br/><a href=\"area.php?pag=insert_statiche_mappa.php\">Riprova</a>";
	echo "<meta http-equiv=\"refresh\" content=\"0;url=area.php?pag=insert_statiche_mappa.php&errore=si&tipoerr=$tipoerr&id=$id&titolo=$titolo&corpo=$corpo&locazione=$locazione&posizione=$posizione&lingua=$lingua&description=$description&keywords=$keywords\" />";
}
elseif (!$id)
{
	...
	// se la nostra query di controllo genera un risultato, generiamo un errore
		...
		//echo "<br/><a href=\"area.php?pag=insert_statiche_mappa.php\">Riprova</a>";
		echo "<meta http-equiv=\"refresh\" content=\"0;url=area.php?pag=insert_statiche_mappa.php&errore=si&tipoerr=$tipoerr&titolo=$titolo&corpo=$corpo&locazione=$locazione&posizione=$posizione&lingua=$lingua&description=$description&keywords=$keywords\" />";
	}
	else
	// se la nostra query di controllo non genera alcun risultato, procediamo con l'inserimento
	{
	$str="INSERT INTO statiche (titolo, description, keywords, corpo, ordine, maps, id_lingua) VALUES ('$titolo', '$description', '$keywords', '$corpo', '$ordine', '$maps', $lingua)";
...
else
{
	...
	$str="UPDATE statiche SET titolo = '$titolo', description = '$description', keywords = '$keywords', corpo = '$corpo', ordine = '$ordine', maps = '$maps', id_lingua = $lingua WHERE ID = $id LIMIT 1";
   // query di modifica
   $risultato=mysql_query($str);
   if (!$risultato)
   ...
   {
   	$str="Select IDmenu from menuvoci where link ='$id' and stat='si' LIMIT 1";
   	$risultato=mysql_query($str);
   	if (mysql_num_rows($risultato)>0)
   	{ 
   		$str=" UPDATE menuvoci set IDmenu = '$menu' WHERE link = '$id' and stat='si' LIMIT 1";
   			// query di modifica per legare notizia ad un menu esistente
   		$risultato=mysql_query($str);
   	}
   	else
