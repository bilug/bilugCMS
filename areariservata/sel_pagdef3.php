...
if (isset($_POST["op"]))
{
	$str = "UPDATE parametri SET valore = './html/static.php' WHERE nomecampo='_CORPO' AND sezione = 0 LIMIT 1";
   ...
   // controllo se la query di inserimento è andata a buon fine
   {
	...
	{	
		...
		$str="UPDATE parametri SET valore = '".apici($_POST["op"])."' WHERE nomecampo = '_STATICADB_$lingua_statica' AND sezione = 5 LIMIT 1";
		$risultato=mysql_query($str);
		
		if (!$risultato)
			echo "ERRORE: Modifica PARAMETRI pagina statica non andato a Buon Fine"; //controllo se la query di inserimento è andata a buon fine
	   else
	   {
...
<?php if ( isset( $_POST['lingua'] ) ) { ?>	
	<br />
	<hr/>
	<form name="modify_pagdef3" method="post" action="./area.php?pag=sel_pagdef3.php">
	<div>
		- Seleziona una pagina statica tra quelle inserite da visualizzare come homepage<br/>	
		<br/>	
		<?php
		$str = "
			SELECT s.ID, s.titolo 
			FROM statiche s 
			INNER JOIN lingue l ON s.id_lingua = l.id 
