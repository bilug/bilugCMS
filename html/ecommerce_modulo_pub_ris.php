...
<?php if ( !isset($_SESSION['arearis']) ) : ?>
	...
	<div class="blocco blocco-ecommerce-login">
		<h3><span>Area riservata</span></h3>	
		<div class="modulo">
			<form name="login" method="post" action="<?=_URLSITO?>/html/ecommerce_guardian.php">
				<p>Username</p>
				<p><input type="text" name="user_ris" class="textlato"/></p>
				<p>Password</p>
				<p><input type="password" name="pwd_ris" class="textlato"/></p>
				<p>
					<input type="submit" value="Login" class="bottomlato"/>
					<?php if ( isset($_SESSION['bilugcms_errore']) ) : ?>
						<?php unset($_SESSION['bilugcms-errore']); ?>
						<span class="bilug-errore">Login Errato</span>
					<?php endif; ?>					
				</p>
			</form>
		</div>
...
<?php else : ?>
	<div class="blocco bilug-login">
		...
		<div class="modulo">
		<?php	
			$str = "SELECT nome, articoli FROM ecommerceris WHERE ID = $_SESSION[arearis] LIMIT 1";
			$risultato = mysql_query($str);
			$control = mysql_fetch_row($risultato);
			$nome_utente = $control[0];
			$id_articoli = substr($control[1], 1, strlen($control[1])-2);
			echo "<p>Utente: $nome_utente</p>";
			?>
